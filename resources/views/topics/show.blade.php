...
@section('content')
    ...
    <button  class="btn btn-info mb-3" onclick="toggleReplyComment({{ $comment->id }})">Répondre</button>
        <form action="{{route('comments.storeReply', $comment)}}"method="Post"
              class="ml-5 d-none" id="replyComment-{{ $comment->id }}">
            @csrf
            ...
            <button type="submit" class="btn btn-primary">Repondre à ce commentaire</button>
        </form>
    @endauth
    @empty
    <div class="alert alert-info">Aucun commentaire pour ce topic</div>
    @endforelse
    <form action="{{route('comments.store', $topic)}}" methode="Post" class="mt-3">
        @csrf
        <div class="form-group">
